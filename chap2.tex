...
$\Delta$ eine \emph{geometrische Realisierung von $K$}. (Siehe
\cref{asc:fig:geomrealization}.) Das Polyeder $\polyeder\Delta$ bezeichnen wir
dann auch als ein \emph{Polyeder von $K$}. (Letzteres ist bis auf Homöomorphie 
eindeutig, siehe etwa Munkres\cite[Ch.\,1,\;\S3,\;3.1(b)]{bookc:munkres84}.)
...
\begin{thBeispiel}[Graphen als abstrakte simpliziale Komplexe]
    Jeder (einfache, ungerichtete) Graph $(V,E)$ mit
    Eckenmenge~$V$ und Kantenmenge~$E$ definiert einen
    eindimensionalen simplizialen Komplex~$(V,K)$, wobei 
    $K\defeq \{ A \Mid A\subset \{u,v\}\in E \}$.
    (Siehe \cref{asc:fig:graphtocomplex}.)
\end{thBeispiel}
...
Variante Munkres\cite[Ch.\,1,\;\S3,\;3.1(a)]{bookc:munkres84}. Stattdessen
...
nötige Dimension $d\in\N$ gibt, für welche es zu einem abstrakten simplizialen
...
\begin{thSatz}%
    ...
    %
    Sei $K$ ein endlicher abstrakter simplizialer Komplex der Dimension 
    $\dim(K)=d\in\N$.
...
\begin{proof}
    ...
    der konvexen Hülle über Teilmengen davon eine injektive Abbildung
    $\powerset\bigl(f(F\cup G)\bigr)\to\powerset\bigl(\R^d\bigr)$ ist.
...
\begin{figure}[b]
    ...
    \caption{Ordnungskomplex $K\defeq \Delta(T)$ (links) zur partiell geordneten 
        Menge $(T,\mkern2mu|\mkern1mu)$ mit $T = \{1,3,5,15\}$
            (wobei \enquote{$\mkern1mu|\mkern2mu$} die Teilerrelation bezeichnet) und eine 
            geometrische Realisierung (rechts)}
...
\begin{figure}[p]
    ...
    \begin{tikzpicture}[%
        ...
        \draw [->, very thick] 
            (9,-0.3) -- node [right=7pt,scale=1.5] {\enquote{$\subset$\kern1pt}} (9,5);
    \end{tikzpicture}
