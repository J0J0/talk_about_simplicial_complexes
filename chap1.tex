...
\begin{thDef}[Seite eines Simplex]
    Ist $d\in\N$ und $\sigma$ ein Simplex mit der Eckenmenge $V \subset\R^d$, so
    ist die konvexe Hülle $\conv(V')$ einer Teilmenge $V'$ von $V$ wieder ein
...
\begin{thDef}%
    ...
    seiner Simplizies:
    \[ \dim(\Delta) \defeq \max\{ \dim(\sigma) \Mid \sigma\in\Delta \} 
        \;\in\N \cup\{-1\}
    . \]
...
\begin{thDef}[Triangulierbar, Triangulierung]
    ...
    $f\colon\polyeder\Delta \isorightarrow X$ gibt. Das Paar $(\Delta,f)$
    nennen wir dann eine \emph{Triangulierung von $X$}.
\end{thDef}
...
\begin{thBeispiel}[Triangulierung des Torus]
    ...
    zuvor miteinander identifizieren\footnote{%
        Streng genommen ist schon diese Indentifikation mit unseren Definitionen
        nicht erlaubt: Nach unserer Definition müssen geometrische Simplizes
        immer \enquote{starr und ungekrümmt} im umgebenden Raum liegen.%
    }, so bilden die dargestellten Dreiecke
